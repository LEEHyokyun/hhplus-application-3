...
@UnitTest
public class OrderRedisTemplateProviderUnitTest {
	
	@Autowired
	OrderRedisTemplateProvider orderRedisTemplateProvider;
	
	@Autowired
	RedisTemplate<String, String> redisTemplate;
	
	@Test
	@DisplayName("[단위테스트] Redis와의 정상적인 통신을 확인하며 Redis에 productId와 점수처리를 확인한다.")
	void hotSaledProductSearchTest() {
		 ...
		 * - 검증 대상의 동작 하나만 기술한다.
		 * */
		orderRedisTemplateProvider.setProductRanking(ProductEnum.HOT_SALE_PRODUCT.key(), 1L, 50L);
		Long actualRanking = redisTemplate.opsForZSet().rank(ProductEnum.HOT_SALE_PRODUCT.key(), 1L);
